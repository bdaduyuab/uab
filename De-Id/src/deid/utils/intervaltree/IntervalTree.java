...
public class IntervalTree<Type> implements Serializable{
	...
	public void remove(Type t) {
		List<Interval<Type>> removeds=new ArrayList<>();
		for(Interval<Type> in:intervalList){
			if(in.getData().equals(t))
				removeds.add(in);
		}
		intervalList.removeAll(removeds);
		inSync = false;
	}
	public void removeAll(Collection<Type> datas) {
		List<Interval<Type>> removeds=new ArrayList<>();
		for(Interval<Type> in:intervalList){
			if(datas.contains(in.getData()))
				removeds.add(in);
		}
		intervalList.removeAll(removeds);
		inSync = false;
